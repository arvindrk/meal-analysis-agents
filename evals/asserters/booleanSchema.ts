...
export function createBooleanAsserter<T extends Record<string, boolean>>(
  ...
  const fields = schema.shape ? (Object.keys(schema.shape) as (keyof T)[]) : null;

  return (output: string, context: { vars: Record<string, unknown> }) => {
    const parsed = JSON.parse(output);
    const result = schema.safeParse(parsed);
    if (!result.success) return { pass: false, score: 0, reason: `Invalid: ${result.error.message}` };
    ...
    return {
      ...
      reason: mismatched.length === 0 ? 'All match' : `Mismatch: ${mismatched.join(', ')}`,
    };
  };
}
